...
test('confirmation token is parsed from inbound reply text', async () => {
  const { processIncomingConfirmationWebhook } = await modulePromise;
  ...
  const result = await processIncomingConfirmationWebhook(payload as any, {
    findPending: async (_from, _contextMessageId, token) => {
      seenToken = token || null;
      return {
        id: 'confirmation-token-1',
        scheduled_post_id: 'scheduled-token-1',
        recipient_phone: '15550009999',
        prompt_message_id: null,
        confirmation_token: 'CF-85A65697',
        media_queue_json: [],
        final_text: 'Caption',
        media_url: 'https://example.com/image.jpg',
        mime_type: 'image/jpeg',
    ...
    recordInbound: async () => ({ eventId: 'evt-token-1', duplicate: false }),
    updateInbound: async () => {},
    onAffirmative: async () => {},
    onAffirmativeFailure: async () => {},
  });
  ...
  assert.equal(result.confirmed, 1);
  assert.equal(seenToken, 'CF-85A65697');
});
